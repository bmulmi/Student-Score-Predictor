...
my_features_names = ['Sex','SATMath','High School GPA', 'Semester Taken']
...
x1 = df['SATMath'].apply((lambda x: custom_round(x, base=50))).values
x2 = df['High School GPA'].round(1).values

#set X as the data
X = pd.DataFrame(df, columns = ['Sex'])
X['SATMath'] = x1
X['High School GPA'] = x2

#set y as target data
y = np.where(df['Semester Taken'] <= str(16.0), 1, 0)
